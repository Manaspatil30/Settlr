...
const ProfileScreen = () => {
  ...
  const handleConnectStripe = async () => {
    ...
    try {
      ...
      Alert.alert('Error', err.response?.data?.error || 'Could not connect Stripe account');
    } finally {
      setConnecting(false);
    }
  };
  ...
  const stripePartial  = stripeStatus?.connected && !stripeStatus?.payoutsEnabled;
      ...
      <View style={styles.card}>
        ...
        {loadingStripe ? (
          ...
          <View style={styles.stripeReady}>
            ...
            <Text style={styles.stripeReadyText}>Ready to receive payments</Text>
          </View>
        ) : (
          <>
...
const styles = StyleSheet.create({
  ...
  stripeReady: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 },
  stripeReadyIcon: { fontSize: 20 },
  stripeReadyText: { fontSize: 15, color: COLORS.success, fontWeight: '600' },
  stripeDesc:   { fontSize: 14, color: COLORS.grey, lineHeight: 20, marginBottom: 14 },
  stripeBtn: {
    backgroundColor: COLORS.primary,
    borderRadius: 12,
    paddingVertical: 14,
